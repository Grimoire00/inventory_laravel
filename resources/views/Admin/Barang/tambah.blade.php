                ...
                <div class="row">
                    <div class="col-md-7">
                        ...
                        <div class="form-group">
                            ...
                            <select name="jenisbarang" class="form-control" onchange="handleJenisBarangChange(this)">
...
@section('formTambahJS')
    <script>
        function handleJenisBarangChange(select) {
            ...
            if (words.length > 1) {
                ...
                initials = selectedText.substring(0, 2).toUpperCase();
            }

            generateIDCreate(initials);
        }

        function generateIDCreate(initials) {
            const timestamp = new Date().getTime();
            $("input[name='kode']").val(initials + "-" + timestamp);
            // console.log("BRG-" + initials + "-" + timestamp);
        }

        function generateIDCreate(initials) {
            const jenisbarang_id = $("select[name='jenisbarang']").val();

            $.ajax({
                type: 'GET',
                url: "{{ route('barang.getLatestCode') }}",
                data: {
                    jenisbarang_id: jenisbarang_id
                },
                dataType: 'json',
                success: function(response) {
                    $("input[name='kode']").val(response.code);
                },
