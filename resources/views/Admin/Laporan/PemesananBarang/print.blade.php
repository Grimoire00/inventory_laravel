...
<body onload="window.print()">
    ...
    <center>
        <h1 class="font-medium">Laporan Pemesanan Barang</h1>
        @if($tglawal == '')
    ...
    <table border="1" id="table1">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{Carbon::parse($d->pesan_tanggal)->translatedFormat('d F Y')}}</td>
                <td>{{$d->pesan_kode}}</td>
                <td>{{$d->barang_kode}}</td>
                <td>{{$d->barang_nama}}</td>
                <td align="center">{{$d->pesan_jumlah}}</td>
                <td align="center">{{$d->pesan_totalharga}}</td>
                <td>{{$d->supplier_nama}}</td>
                <td>{{$d->pesan_status}}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
